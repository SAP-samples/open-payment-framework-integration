...
**In summary**: to import the [Adyen Component Bruno Collection](https://github.com/SAP-samples/open-payment-framework-integration) this page will guide you through the following steps:
...
d) Prepare Bruno Environment Variables so the collection can be imported with all your OPF tenant and Adyen test account unique values.
...
## Setting up Your Adyen Test Account to Work with OPF
   ...
   Once you have created your Adyen test account, do the following to set it up to work with OPF:
...
The client key is used to authenticate requests from your payment environment which renders input fields for card details,
so there should be one or more allowed origins for the client key.
...
| service                                      | The ``service`` is the name of your OPF service in the specific environment. The value is ``opf`` by default.                                                                                                                                                                                                                                                                                                           |
...
| serviceVersion                               | v71                                                                                                                                                                                                                                                                                                                                                                                                                     |
| clientKey                                    | Go to **Developers** -> **API credentials** -> **ws User** to copy the Client Key.                                                                                                                                                                                                                                                                                                                                      |
| checkoutShopperHost                          | ``checkoutshopper-test.cdn.adyen.com``                                                                                                                                                                                                                                                                                                                                                                                  |
| mode                                         | ``test``                                                                                                                                                                                                                                                                                                                                                                                                                |
| sdkVersion                                   | ``6.22.0`` — it is recommended to use the latest version                                                                                                                                                                                                                                                                                                                                                                |
| nativeThreeDS                                | ``preferred`` — please refer to the Adyen documentation for more information                                                                                                                                                                                                                                                                                                                                            |
...
| applePay**                                   | These parameters are for the OPF Quick Buy function. If you prefer to use ApplePay within the Adyen Web Component, please leave these items unchanged and ensure they are unselected in your Bruno collection.                                                                                                                                                                                                          |

**applePay Variables**

To enable Apple Pay with Adyen in OPF, you need two developer accounts: an Adyen developer account and an Apple developer account.

You also need to follow [Use your own certificate for web in Adyen](https://docs.adyen.com/payment-methods/apple-pay/apple-pay-certificate/web) to:

* Create your certificate (.p12 file with a password).
* Register and validate your merchant domain.

After completing the above steps, you can retrieve your ``applePayMerchantId`` and ``applePayMerchantName`` from your Apple Developer account.

![](../images/applepay_vars.png)

The ``applePayMerchantPassword`` is the password for the .p12 file.
...
![](../images/applepay_account.png)

Upload the .p12 file to your configuration.

![](../images/applePay_p12.png)


## Payment Methods Supported in OPF

a) The list of the default supported card types in OPF:
...
For other card types, you need to contact the SAP OPF support team to add the new card type.

b) Alternative Payment Method

OPF supports many APMs, such as `paypal`, `klarna`, and `afterpay`. However, these APMs are not automatically activated upon importing Bruno collections. Instead, you'll need to carry out the following steps in your OPF workbench:
...
Step 3:
Check if the APM is mapped correctly in Configuration Components (Notification).
